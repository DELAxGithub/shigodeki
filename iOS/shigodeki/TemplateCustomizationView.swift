...
struct TemplateCustomizationView: View {
    ...
    private var templateOverviewSection: some View {
    ...
    private var phaseColorCustomizationSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("フェーズカラー")
                .font(.subheadline)
                .fontWeight(.medium)
    ...
    private var taskPriorityCustomizationSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("タスク優先度の調整")
                .font(.subheadline)
                .fontWeight(.medium)
    ...
    private func createCustomizedProject() {
        ...
        // カスタマイズされたテンプレートを作成
        let customizedTemplate = applyCustomizations(to: template, with: finalCustomizations)
        
        onConfirm(customizedTemplate)
    }
    
    private func applyCustomizations(to template: ProjectTemplate, with customizations: ProjectCustomizations) -> ProjectTemplate {
...
struct TemplateCustomizationView_Previews: PreviewProvider {
