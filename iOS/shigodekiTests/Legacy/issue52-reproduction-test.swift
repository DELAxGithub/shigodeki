...
class MockMemberDetailViewModel {
    ...
    // Mock data storage (simulated Firestore)
    private var allTasks: [MockTask] = []
    private var allMembers: [MockMember] = []
    ...
    private func simulateTaskQuery(memberId: String) -> Result<[MockTask], String> {
        // Issue #52 Bug Scenarios:
        
        // Scenario 1: Query syntax/structure error (most likely)
        ...
        case 1:
            ...
            return .failure("Query failed: field 'assignedTo' does not exist")
            
        case 2:
            // Bug: Permission/security rule issue
            print("    🐛 Simulating permission denied error")
            return .failure("Permission denied: insufficient access to tasks collection")
    ...
    func assignTaskToMember(taskId: String, memberId: String) {
        ...
        if var task = allTasks.first(where: { $0.id == taskId }) {
...
struct Issue52ReproductionTest {
    ...
    func testMemberAssignedTasksNotDisplayed() {
        ...
        print("    Query error: \(viewModel.taskQueryError ?? "none")")
        
        let memberLoaded = viewModel.member != nil
        let hasTaskQueryError = viewModel.taskQueryError != nil
        let noAssignedTasksDisplayed = viewModel.assignedTasks.isEmpty
        ...
        if memberLoaded && expectedTasksExist && (hasTaskQueryError || noAssignedTasksDisplayed) {
            ...
            if hasTaskQueryError {
                print("         Cause: Task query error - \(viewModel.taskQueryError!)")
            } else {
    ...
    func testTaskAssignmentFlow() {
        ...
        print("    Assignment visible: \(viewModel.assignedTasks.contains { $0.id == unassignedTask.id })")
        
        let assignmentVisible = viewModel.assignedTasks.contains { $0.id == unassignedTask.id }
    ...
    func testMultipleTaskAssignments() {
        print("\n🧪 Test Case: Multiple Task Assignments")
        
        // Arrange
        let viewModel = MockMemberDetailViewModel()
        let testMember = viewModel.allMembers.first!
        
        print("  Testing multiple task assignments:")
        
        // Act: Assign multiple tasks
        for task in viewModel.allTasks.prefix(3) {
            viewModel.assignTaskToMember(taskId: task.id, memberId: testMember.id)
        }
        ...
        print("  Results:")
        print("    Expected assigned tasks: \(expectedCount)")
        print("    Displayed assigned tasks: \(displayedCount)")
        print("    Query successful: \(viewModel.taskQueryError == nil)")
        
        if displayedCount == expectedCount && viewModel.taskQueryError == nil {
            print("  ✅ PASS: Multiple task assignments displayed correctly")
        } else if viewModel.taskQueryError != nil {
            print("  ❌ FAIL: Query error prevents multiple assignments from being displayed")
        } else {
            print("  ❌ FAIL: Not all task assignments are displayed")
            print("         Expected: \(expectedCount), Got: \(displayedCount)")
        }
    }
...
testSuite.testMultipleTaskAssignments()
