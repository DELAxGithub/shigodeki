//
//  PHASE2_PERFORMANCE_VALIDATION.swift
//  shigodeki
//
    ...
    private func runPerformanceTests() {
        ...
        Task {
            await InstrumentsSetup.shared.runMeasurementScenario(.memoryStress) {
                // メモリストレステストの実行
                for i in 0..<10 {
                    let _ = sharedManagers.projectManager
                    let _ = sharedManagers.taskManager
                    let _ = sharedManagers.phaseManager
                    
                    await Task.sleep(UInt64(100_000_000)) // 0.1秒待機
                    
                    if i % 3 == 0 {
                        sharedManagers.cleanupUnusedManagers()
                    }
                }
            }
